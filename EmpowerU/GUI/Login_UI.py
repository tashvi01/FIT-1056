...
class LoginScreen(tk.Frame):
    def __init__(self, master):
        ...
        self.label = tk.Label(master=self, image=self.logo, width=750, height=300)  
        self.label.grid(row=0, column=0, sticky=tk.S, padx=10, pady=10) 

        # Welcome heading
        self.welcome_label = tk.Label(master=self, text="Welcome to EmpowerU", font=("Arial Bold", 20))
        self.welcome_label.grid(row=1, column=0, pady=10)

        # Create buttons below the welcome label
        self.register_button = tk.Button(master=self, text="Register", command = self.open_register)
        self.register_button.grid(row=2, column=0, padx=10, pady=10)

        self.signin_button = tk.Button(master=self, text="Sign In")
